...
/*   Updated: 2023/03/10 17:34:45 by ikhabour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_images(t_map data)
{
	...
	{
		put_image(data, "./textures/grass_path.xpm", data.x, data.y);
		put_image(data, "./bonus/textures/enemy/right_flight1.xpm", data.x,
			data.y);
	}
	else if (data.map[data.y][data.x] == 'P')
	{
		put_image(data, "./textures/grass_path.xpm", data.x, data.y);
		put_image(data, "./textures/player/down/down_idle.xpm", data.x, data.y);
	}
}
...
void	after_move(t_map *data, t_cords *player_pos, char *move)
{
	...
	{
		data->current_tile = 'V';
		if (data->map[player_pos->y][player_pos->x] == 'C')
			(data->c)--;
		data->map[player_pos->y][player_pos->x] = 'P';
		render_player(data, player_pos, move);
	}
	else if (data->map[player_pos->y][player_pos->x] == 'X')
		you_lost();
}
